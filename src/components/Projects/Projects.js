...
function Projects() {
  return (
    <section
      id="projects"
      className="flex justify-center bg-gray-900 dark:bg-gray-200"
    >
        ...
        <div className="text-center mb-12 mx-auto">
          <RubberBand>
            ...
            <p className="text-base text-gray-300 dark:text-gray-600 font-mono leading-relaxed xl:w-2/4 md:w-3/4 mx-auto">
              Go on, have a little scroll, check out my websites and
              applications to see what I've worked on so far...
            </p>
          </RubberBand>
        </div>
        {project.map((project) => (
                ...
                <div className="px-8 py-10 relative z-10 w-full border-4 border-gray-800 dark:border-gray-400 bg-gray-900 dark:bg-gray-200 opacity-0 hover:opacity-90 transform transition duration-500 ease-in-out">
                  ...
                  <div className="flex flex-wrap pb-2">
                    {project.libraries.map((libraries) => (
                      <h2
                        className="px-1 mx-1 border border-solid border-green-600 rounded-md text-green-500 font-mono text-xs"
                        key={libraries}
                      >
                        {libraries}
                      </h2>
                  ...
                  <p className="leading-relaxed text-gray-200 dark:text-gray-900 text-sm mb-4">
                    {project.description}
                  </p>
                  <form action={project.source} target="_blank">
                    <input
                      type="submit"
                      value="Source Code"
                      className="font-mono text-green-500 border border-solid border-green-500 bg-gray-900 dark:bg-gray-200 hover:bg-green-500 dark:hover:bg-green-500 hover:text-gray-900 focus:outline-none transform hover:scale-110 transition duration-300 ease-in-out p-2 cursor-pointer"
                    />
                  </form>
                </div>
              </div>
